...
from cti_app.domain.discovery import DiscoveryCandidate, same_publication
...
def _signals(
    ...
    shared("shared_canonical_url", {item.id: _urls(item) for item in candidates})
    shared("shared_source_domain", {item.id: _domains(item) for item in candidates})
    for left, right in combinations(candidates, 2):
        for left_source in left.evidence.sources:
            match = next(
                (
                    right_source
                    for right_source in right.evidence.sources
    ...
    shared("shared_ioc", {item.id: _normalized(_ioc_values(item)) for item in candidates})
    ...
    dated = [(item, _candidate_date(item)) for item in candidates]
    ...
    ioc_counts = {item.id: len(_ioc_values(item)) for item in candidates}
...
def _ioc_values(candidate: DiscoveryCandidate) -> tuple[str, ...]:
    evidence = candidate.evidence
    values = [*evidence.iocs]
    values.extend(ioc.normalized_value or ioc.raw_value for ioc in evidence.provisional_iocs)
...
def _candidate_date(candidate: DiscoveryCandidate) -> date | None:
