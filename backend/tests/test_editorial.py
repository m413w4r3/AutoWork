...
class _CanonicalCandidateProjection:
    ...
    async def list_for_edition(
        ...
        snapshot = self._factory.snapshots.get(edition_id)
        if snapshot is None:
            return []
        candidates: list[DiscoveryCandidate] = []
        for subject in snapshot.subjects:
            for index, reference in enumerate(subject.member_references, 1):
...
class _EditorialFactory:
    def __init__(self) -> None:
        self._factory = InMemoryEditorialUnitOfWorkFactory()
        self.editions = self._factory.editions
        self.snapshots = self._factory.snapshots
        self.groups = self._factory.groups
        self.subjects = self._factory.subjects
        self.decisions = self._factory.decisions

    def __call__(self):
        uow = self._factory()
        uow.discovery_candidates = _CanonicalCandidateProjection(self._factory)
        return uow


def _factory() -> _EditorialFactory:
    ...
    uow.snapshots[edition.id] = _snapshot(
        ...
        [
            (
                first_id,
                first,
                (DiscoveryMemberReference(first.id),),
            ),
            (
                second_id,
                second,
                (DiscoveryMemberReference(second.id),),
            ),
        ],
    )
...
async def test_snapshot_enrichment_keeps_selected_editorial_group_and_subject() -> None:
    ...
    first_ref = DiscoveryMemberReference(first.id)
    ...
    second_ref = DiscoveryMemberReference(update.id)
    uow.snapshots[edition.id] = _snapshot(
        edition.id,
