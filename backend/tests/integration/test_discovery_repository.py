...
async def test_discovery_candidate_repository_is_canonical_and_hides_superseded(
    ...
    try:
        ...
        async with uow_factory() as uow:
            ...
            assert [item.candidate.title for item in listed] == ["Same title", "Same title"]
            await uow.discovery_candidates.add_sequence(
                [
                    DiscoveryCandidate(
                        id=replacement_topic.id,
                        discovery_run_id=discovery_run.id,
                        discovery_batch_id=correction.id,
                        position=1,
                        candidate=replacement_topic,
                        supersedes_candidate_id=original_ids[0],
                    )
                ]
            )
            await uow.commit()
        ...
        assert {item.id for item in active} == {original_ids[1], replacement_topic.id}
        assert {item.id for item in history} == {*original_ids, replacement_topic.id}
        assert historical is not None and historical.candidate.title == "Same title"

        # A candidate can only be superseded by one direct replacement.
        with pytest.raises(IntegrityError):
            async with uow_factory() as uow:
                duplicate_replacement = _canonical_topic("Duplicate replacement")
                duplicate_replacement_id = uuid4()
                duplicate_replacement.id = duplicate_replacement_id
                await uow.discovery_candidates.add_sequence(
                    [
                        DiscoveryCandidate(
                            id=duplicate_replacement_id,
                            discovery_run_id=discovery_run.id,
                            discovery_batch_id=correction.id,
                            position=2,
                            candidate=duplicate_replacement,
                            supersedes_candidate_id=original_ids[0],
                        )
                    ]
                )
...
def _canonical_batch(
    ...
    return DiscoveryBatch(
        ...
        contributions=[
            DiscoveryContribution(
                candidate=_canonical_topic(title),
                status=ContributionStatus.ACCEPTED,
                created_at=now,
                accepted_at=now,
            )
