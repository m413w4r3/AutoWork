...
def upgrade() -> None:
    op.create_table(
        ...
        sa.UniqueConstraint("sample_id", "tool_version", "ruleset_sha256", "parameters_sha256", name="uq_capability_sets_replay"),
    )
    op.create_index("ix_capability_sets_blob_id", "capability_sets", ["blob_id"])
    op.add_column("sample_feature_index", sa.Column("capability_set_id", postgresql.UUID(as_uuid=True), nullable=True))
    op.create_foreign_key("fk_sample_feature_index_capability_set", "sample_feature_index", "capability_sets", ["capability_set_id"], ["id"], ondelete="RESTRICT")
    op.alter_column("sample_feature_index", "feature_set_id", existing_type=postgresql.UUID(as_uuid=True), nullable=True)
